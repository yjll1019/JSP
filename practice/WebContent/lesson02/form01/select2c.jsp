	...
	<div class="container">
		<form method="get">
			...
			<div class="form-group">
				<label>number1</label>
				<input type="text" name="number1" class="form-control" value="<%=number1 %>" /> <!-- 파라미터가 없으면 0 출력! -->
			</div>

			<div class="form-group">
				<label>operator:</label> <select name="cmd" class="form-control">
					<option value="+" <%= "+".equals(cmd)? "selected":"" %>>+</option><!-- cmd값이 +이면 select에서 +가보이도록! -->
					<option value="-"<%= "-".equals(cmd)? "selected":"" %>>-</option><!-- selected 애트리뷰트가 있는 부호가 보여지게 된다! -->
			...
			<div class="form-group">
				<label>number2</label> <input type="text" name="number2" class="form-control" value="<%=number2 %>" /><!-- 파라미터가 없으면 0 출력! -->
			</div>
