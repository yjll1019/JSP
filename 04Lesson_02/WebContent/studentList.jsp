...
<%
	...
	if(od==null) od="0";
	List<Student> list = StudentDAO.findByName(name,currentPage, pageSize,od); //이름으로 조회한 결과 리스트만 가져옴
...
<a id="createButton" class="btn btn-primary pull-right" href="studentCreate.jsp?pg=<%= currentPage %>&name=<%=nameEncoded%>&od=<%=od%>">
...
<table class="table table-bordered table-condensed">
	...
	<tbody> <!-- 검색한 이름, 현재 페이지 수가 학생 수정 후 목록으로 나올 때 처음으로 돌아가 있으면 안되기 때문에 url로 정보 전달. -->
		<% for(Student s : list) {%>
		<tr data-url="studentEdit.jsp?id=<%= s.getId() %>&pg=<%=currentPage%>&name=<%=nameEncoded%>&od=<%=od%>">
			<td><%=s.getId() %></td>
