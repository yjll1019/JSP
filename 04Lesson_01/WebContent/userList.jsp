...
List<User> list = UserDAO.findByName(name, currentPage, pageSize);
...
<div class="container">
	<h1>사용자 목록</h1>
<a id="createButton" class="btn btn-primary pull-right" href="userCreate.jsp?pg=<%= currentPage %>&name=<%=nameEncoded%>">
...
<form class="form-inline">
	...
	<table class="table table-bordered table-condensed">
		...
		<tbody>
			<% for(User u : list) {%>
					<tr data-url="userEdit.jsp?id=<%= u.getId() %>&pg=<%=currentPage%>&name=<%= nameEncoded%>">
				<td><%= u.getUserid() %></td>
				<td><%=u.getName() %></td>
				<td><%= u.getEmail() %></td>
